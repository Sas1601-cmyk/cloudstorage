...
import ru.forkin.springcourse.cloudstorage.storage.dto.OutputFileDto;
...
public class FileStorageService {
    ...
    public OutputStorageDto moveResourceByPathFromToPathTo(String pathFrom, String pathTo, String username) {
        String prefix = getPrefix(username);
        String normalizedTo;
        if (pathTo.isEmpty()) {
            String sourceName = pathFrom.endsWith("/")
        ...
        } else {normalizedTo = pathTo.endsWith("/") ? pathTo : pathTo + "/";}
        ...
        if (isSourceDir) {
            ...
            return new OutputFileDto(
                    getFilePath(normalizedTo),
                    getFileName(normalizedTo),
                    statFrom.size());}
    }
    ...
    private void moveFileToNewPlace(String sourceObject, String destObject) {
        try { minioClient.copyObject(CopyObjectArgs.builder()
            ...
            throw new StorageException("Storage error for rename file");}}
    ...
    public void createNewRootDirectoryForNewUser(String username){
        String fullPath = getPrefix(username);
        if(!isExistsDirectory(fullPath)){
            try{ minioClient.putObject(
                    PutObjectArgs.builder().bucket(bucketName).object(fullPath)
                            .stream(new ByteArrayInputStream(new byte[0]), 0, -1)
                            .contentType("application/x-directory").build());
            }catch (Exception e){throw new StorageException("Storage error for create root directory for person");}}}
    ...
    private void initBucket(){
        ...
        }catch (Exception e){ throw new RuntimeException("Failed to initialize storage bucket");}}
}